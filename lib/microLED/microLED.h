...
static void mSystemUptimePoll(void);	// дёрнуть миллисы
...
protected:
    ...
    uint8_t _width = 0, _height = 0, _matrixConfig = 0, _matrixType = 0;
    uint8_t _matrixW = 0;
    int _maxCurrent = 0;
...
public:
    ...
    */
    ...
    ImicroLED(uint8_t width, uint8_t height, M_type type, M_connection conn, M_dir dir) :
...
public:
    ...
    void fillGradient(int from, int to, mData color1, mData color2) {
        for (int i = from; i < to; i++) leds[i] = mGetBlend(i - from, to - from, color1, color2);
    }
    ...
    void fade(int num, byte val) {
        leds[num] = mGetFade(leds[num], val);
    }
    ...
    void fade(int x, int y, byte val) {
        ...
        leds[pix] = mGetFade(leds[pix], val);
    }
    ...
    void send(mData color, byte thisWhite = 0) {
        ...
        if (uptime && (isr == CLI_AVER || isr == CLI_HIGH)) mSystemUptimePoll();  // пнуть миллисы
    }
...
static void mSystemUptimePoll(void) {
